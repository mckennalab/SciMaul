...
class TracebackMatrix (dimX: Int, dimY: Int) {
  ...
  def printMatrix(pad: Int = 7): Unit = {
    println(Matrix.padInt(-1, pad) + (0 until dimX).map { i => Matrix.padInt(i, pad) }.mkString(""))
    (0 until dimY).foreach { indexB => {
      print(Matrix.padInt(indexB, pad))
      (0 until dimX).foreach { indexA => {
        print(Matrix.padString(values(indexA)(indexB).str, pad))
      }
      }
      println()
    }
...
object TracebackMatrix {
  ...
  def tracebackGlobalAlignment(sequenceA: String,
                               sequenceB: String,
                               matFunction: (EmissionState => DenseMatrix[Double]),
    ...
    EmissionState.knownStates.foreach{state =>
      if (matFunction(EmissionState.stringToState(state))(indexA,indexB) < matFunction(EmissionState.stringToState(state))(indexA,indexB))
    ...
    new Alignment {
      ...
      def getScore = {
        EmissionState.knownStates.map{state => matFunction(EmissionState.stringToState(state))(sequenceA.size, sequenceB.size)}.max
      }
